...
function ReviewBody({ review }: { review: StoreReviewSnapshot["reviews"][number] }) {
  ...
  useEffect(() => {
    ...
    if (!element) return;
    const measure = () => setOverflows(element.scrollHeight > parseFloat(getComputedStyle(element).lineHeight) * 5 + 1);
  ...
  return <>
    {review.text ? <blockquote ref={content} id={contentId} className={expanded ? "" : "store-review-collapsed"}>{review.text}{review.excerpt && "…"}</blockquote> : <p className="store-review-empty">這位顧客只留下評分。</p>}
    {overflows && <button type="button" className="store-review-expand" aria-expanded={expanded} aria-controls={contentId} onClick={() => setExpanded(value => !value)}>{expanded ? "收合" : review.excerpt ? "展開內容" : "展開全文"}<svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.6" aria-hidden="true"><path d={expanded ? "m4 10 4-4 4 4" : "m4 6 4 4 4-4"} /></svg></button>}
    {(review.translated || review.excerpt) && <small className="store-review-attribution">{[review.translated && "Google 翻譯", review.excerpt && "先前節錄，全文待更新"].filter(Boolean).join("・")}</small>}
  </>;
}
...
export default function StoreReviews({ snapshot, compact = false, tone = "yellow" }: { snapshot: StoreReviewSnapshot; compact?: boolean; tone?: "yellow" | "coral" }) {
  ...
  return <>
    ...
    <header className="store-reviews-header"><div><span className="eyebrow">GOOGLE REVIEWS</span><h2 id={titleId}>{snapshot.storeName}</h2><p><span className="store-rating-star" aria-hidden="true">★</span> <b>{snapshot.rating.toFixed(1)}</b><span> · {snapshot.reviewCount.toLocaleString("en-US")} 則評價</span></p><small><time dateTime={snapshot.capturedAt}>{capturedDate}</time> 擷取</small></div><button type="button" className="store-reviews-close" aria-label="關閉評論" onClick={() => setOpen(false)} autoFocus><svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.7" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18" /></svg></button></header>
