...
export default function LiveResults({ catalog }: { catalog: Catalog }) {
  ...
  return <section className="live-section arcade-section" id="results" aria-labelledby="results-title">
    ...
    <div className="battle-summary">
      <div className="vote-total" aria-live="polite" aria-atomic="true"><strong>{available ? String(total).padStart(2, "0") : "—"}</strong><span>人已投票{catalog.settings.expectedVoters > 0 && <small>預計 {catalog.settings.expectedVoters} 人</small>}</span></div>
    ...
    <div className="results-grid" data-duel={teams.length === 2} style={boardStyle}>
      {teams.length === 2 && <span className="score-versus" aria-hidden="true">VS</span>}
